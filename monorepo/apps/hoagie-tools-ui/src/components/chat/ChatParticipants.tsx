...
import { TwitchClient, UserData, UserFollows } from '@hoagie/service-clients';
...
export const ChatParticipants = (props: ChatParticipantsProps) => {
  ...
  const trimUsers = (users: ChatUser[]) => {
    ...
    setUsersInChat(
      users.filter(
        (user) => now.getTime() - user.lastChat.getTime() < userExpiryMillis
      )
    );
  };
  ...
  useEffect(() => {
    async function addUserToChat() {
      if (lastMessage && state.streamerData && props.twitchClient) {
        ...
        const userData = await props.twitchClient.getUserData(user);
        if (userData) {
          const existingUser = usersInChat.find(
            (u) => u.userData.id === userData.id
          );
          if (existingUser) {
            setUsersInChat([
              {
        ...
        <FlexCol>
          {usersInChat.map((user) => (
            <FlexRow className="chatusers-row" justifyContent="space-between">
              ...
              {user.userFollows && (
                <div className="chatusers-follow">Follows</div>
              )}
            </FlexRow>
          ))}
        </FlexCol>
